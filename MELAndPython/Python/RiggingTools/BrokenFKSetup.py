...
# create constraints
p_constraint_1 = cmds.parentConstraint(parent_ctrl, child_ctrl_grp, mo=True, skipRotate=['x','y','z'], weight=1)[0] #constrains translate
p_constraint_2 = cmds.parentConstraint(parent_ctrl, child_ctrl_grp, mo=True, skipTranslate=['x','y','z'], weight=1)[0] #constrains rotations
cmds.scaleConstraint(parent_ctrl, child_ctrl_grp, offset=(1,1,1), weight=1) #constrains scaling
...
cmds.connectAttr('%s.FollowTranslate' % child_ctrl,'%s.w0' % p_constraint_1, f=True)
cmds.connectAttr('%s.FollowRotate' % child_ctrl,'%s.w0' % p_constraint_2, f=True)